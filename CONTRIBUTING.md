...
`README.md`（英文）与 `README.zh-CN.md`（简体中文）是同一份文档的两个语言版本：

- 改动其中一份，必须同步改动另一份，否则 CI 会失败
- 两份都要保留顶部的语言切换链接，以及完整的安装章节
- 安装命令必须与实际脚本一致 —— 改 `install.mjs` 的参数时，别忘了改 README 里的参数表
- 不要新增第三、第四种语言的 README（除非你愿意长期维护）
